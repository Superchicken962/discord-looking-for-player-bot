...
class AlertData extends DataFileManager {
    ...
    async unregister(opts) {
        ...
        if (gData.length === 0) {
            throw new Error("Can't delete an alert when none exist!");
        };

        // Similiarly, if alert is not found in existing data just return true.
        const alertIndex = gData.findIndex(a => a.name === opts.name);
        if (alertIndex === -1) {
            throw new Error("Alert by that name not found! Ensure it is the same, the name is case-sensitive!");
        };
